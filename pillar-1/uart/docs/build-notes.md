...
Worked example, depth=8: addressing 8 slots needs exactly 3 bits ($2^3=8$). With *only* 3-bit pointers: start empty at read=write=3. Push 8 bytes, write_ptr wraps 3→4→...→7→0→1→2→**3** again (mod-8 arithmetic) — identical to read_ptr. Empty and completely-full now produce the same comparison; 3-bit pointers can't tell "0 apart" from "one full lap (8) apart," since mod-8 makes 0 and 8 indistinguishable in 3 bits.

Fix: widen both pointers to 4 bits, but only use the bottom 3 (`ptr[2:0]`) as the actual memory address — let the full 4-bit value count to 15 before wrapping. Redo: read=3 (0011), push 8, write becomes 11 (1011). As 4-bit values, 3 ≠ 11 (distinguishable), but `11[2:0] = 3` (still addresses the right slot). Rule falls out naturally: **EMPTY** = pointers fully equal, all 4 bits. **FULL** = bottom 3 bits equal, but the extra bit differs.
